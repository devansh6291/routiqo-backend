...
# --- TOMTOM ROUTING ENGINE (NO SAFETY NET) ---
def get_tomtom_geometry(start_lat, start_lng, end_lat, end_lng):
    """
    Directly queries TomTom for the Polyline (Shape) of the road.
    """
    base_url = "https://api.tomtom.com/routing/1/calculateRoute"
    locations = f"{start_lat},{start_lng}:{end_lat},{end_lng}"
    
    # We explicitly ask for 'polyline' to get the shape
    url = f"{base_url}/{locations}/json?traffic=true&routeRepresentation=polyline&key={TOMTOM_API_KEY}"
    
    # This will RAISE an error if the request fails (as requested)
    response = requests.get(url)
    data = response.json()
    
    # Parse the route shape
    points = data['routes'][0]['legs'][0]['points']
    # Convert to [[lat, lng], [lat, lng]] format for Leaflet
...
def run_optimization(strategy: str = "balanced", vehicles: int = 3):
    with SessionLocal() as db:
        ...
        for i in range(min(vehicles, len(order_chunks))):
            ...
            # Container for the huge list of road points
            route_geometry = []
            stops_metadata = [{"lat": current_lat, "lng": current_lng, "type": "HUB", "name": main_hub['name']}]
            
            # 2. Visit Spokes
            for order in chunk:
                # CALL TOMTOM: Get exact road shape to next stop
                try:
                    shape = get_tomtom_geometry(current_lat, current_lng, order['lat'], order['lng'])
                    route_geometry.extend(shape)
                except Exception as e:
                    print(f"TomTom API Failed: {e}")
                    # If it fails, we add a straight line just to prevent a total crash, 
                    # but the console will log the failure.
                    route_geometry.append([current_lat, current_lng])
                    route_geometry.append([order['lat'], order['lng']])
            ...
            try:
                shape_back = get_tomtom_geometry(current_lat, current_lng, main_hub['lat'], main_hub['lng'])
            ...
            routes.append({
                ...
                "geometry": route_geometry, # This goes to Leaflet
