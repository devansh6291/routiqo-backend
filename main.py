...
# --- CORE LOGIC: HUB & SPOKE + STRATEGY COLORING ---
...
def run_optimization(strategy: str = "balanced", vehicles: int = 3):
    with SessionLocal() as db:
        ...
        # 1. Select Color Theme based on STRATEGY (User Requirement #4)
        if strategy == "parking":
            # Green Shades (Parking First)
            palette = ['#10b981', '#059669', '#34d399', '#064e3b']
        elif strategy == "speed":
            # Blue Shades (Time First)
            palette = ['#3b82f6', '#2563eb', '#60a5fa', '#1e40af']
        else:
            # Orange/Purple (Balanced)
            palette = ['#f59e0b', '#8b5cf6', '#d97706', '#7c3aed']

        routes = []
        main_hub = hubs[0] # All routes radiate from Central Depot
        
        # 2. Assign Orders to Vehicles (User Requirement #8)
        # Limit the chunks by the number of vehicles requested
        ...
        for i in range(min(vehicles, len(order_chunks))):
            ...
            routes.append({
                "vehicle_id": f"V-{i+1}",
                "color": palette[i % len(palette)], # Apply Strategy Color
                "stops": stops,
...
def get_tracking():
    ...
    with SessionLocal() as db:
        ...
        for v in vehs:
            ...
            results.append({"id": v.id, "name": v.name, "lat": new_lat, "lng": new_lng, "status": v.status, "type": v.type})
        return results
...
def get_stats():
    with SessionLocal() as db:
        ...
        # New Rich Data for Dashboard
        return {
            "on_time_rate": "97.5%",
            "hub_efficiency": "92%",
            "active_vehicles": active,
            "pending_orders": pending,
            "co2_saved": "142 kg", # Sustainability Metric
            "alerts": ["Heavy traffic in North Zone", "Alpha-Van Low Fuel"],
            "hub_load": [ # For Progress Bars
                {"name": "Central Depot", "percent": 75},
                {"name": "North Warehouse", "percent": 40}
            ],
            "graph_data": [12, 19, 15, 25, 22, 30, 28], # Fake Weekly Savings Data
            "recent_logs": [ # Live Feed Data
