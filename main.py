...
# TOMTOM KEY (Used ONLY for visual Traffic Tiles now)
...
def get_coords_from_address(address):
    ...
    return 22.7196, 75.8577 # Fallback to Indore Center

# --- OSRM ROUTING (Street-Level Geometry) ---
def get_osrm_geometry(start_lat, start_lng, end_lat, end_lng):
    """
    Fetches exact street path from OpenStreetMap (OSRM).
    """
    try:
        ...
        if resp.status_code == 200:
            ...
            # OSRM returns [Lng, Lat], Leaflet needs [Lat, Lng] -> SWAP
...
class OrderCreate(BaseModel):
    customer: str
    address: str
    # Lat/Lng are now OPTIONAL (calculated from address if missing)
    lat: float = 0.0
    lng: float = 0.0
    priority: int
    time_window: str
...
def run_optimization(strategy: str = "balanced", vehicles: int = 3):
    with SessionLocal() as db:
        ...
        # 1. STRATEGY: SORT ORDERS
        # "Time First" -> Simple Nearest Neighbor (simulated by sorting by Lat)
        # "Balanced" -> Default ID sort
        if strategy == "speed": # Time First
            orders.sort(key=lambda x: x['lat']) 
        elif strategy == "parking": # Parking First (Simulated by Priority)
            orders.sort(key=lambda x: x['priority'], reverse=True)
            
        # Color Palettes
        if strategy == "parking": palette = ['#10b981', '#059669'] # Green
        elif strategy == "speed": palette = ['#3b82f6', '#2563eb'] # Blue
        else: palette = ['#f59e0b', '#8b5cf6'] # Orange/Purple
        ...
        for i in range(min(vehicles, len(order_chunks))):
            ...
            # 3. BUILD ROUTE WITH OSRM
            for order in chunk:
                # Get precise street path
