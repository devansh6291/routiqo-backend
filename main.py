...
def run_optimization(strategy: str = "balanced", vehicles: int = 3):
    ...
    if not orders:
        orders = [
            ...
            {'customer': 'Bhawarkua Store', 'lat': 22.6930, 'lng': 75.8620, 'time_window': '10:00', 'priority': 3},
            {'customer': 'Palasia Mall', 'lat': 22.7250, 'lng': 75.8900, 'time_window': '11:00', 'priority': 4}
        ]
    if not hubs:
        hubs = [{'name': 'Central Depot', 'lat': 22.7196, 'lng': 75.8577}]

    # 3. DISTINCT OPTIMIZATION LOGIC (Fixing the "Same Route" issue)
    if strategy == "speed": 
        # Sort North-to-South (Latitude)
        orders.sort(key=lambda x: x['lat'], reverse=True)
        palette = ['#3b82f6', '#2563eb', '#60a5fa'] # BLUE THEME
        
    elif strategy == "parking": 
        # Sort by Priority (High Prio First)
        orders.sort(key=lambda x: x['priority'], reverse=True)
        palette = ['#10b981', '#059669', '#34d399'] # GREEN THEME
        
    else: 
        # "Balanced": Random Shuffle (Seeded so it's consistent per run, but different from others)
        random.seed(42) 
        random.shuffle(orders)
        palette = ['#f97316', '#a855f7', '#eab308', '#ec4899'] # ORANGE / PURPLE / YELLOW
    
    # Ensure vehicle count doesn't crash division
    ...
    for i in range(min(vehicles, len(order_chunks))):
        ...
        for order in chunk:
            # OSRM Call with Timeout Protection
            ...
            try:
                ...
                resp = requests.get(url, timeout=1.0) # 1 sec max
                if resp.status_code == 200:
                    coords = resp.json()['routes'][0]['geometry']['coordinates']
                    path = [[p[1], p[0]] for p in coords] # Swap Lng/Lat
            except: pass
